...
def cached_tools() -> list:
    ...
    live call would have succeeded. That is the normal case on a fresh deploy,
    where the agent starts before its run.invoker binding on the data plane has
    propagated -- the probe fails once, and the stack then looks broken long
    after it is fine.
    """
    with _TOOLS_LOCK:
